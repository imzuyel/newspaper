...
@section('title') Addvertiesment Add @endsection
@section('content')
<section class="content">

    <div class="container-fluid">
        <ol class="breadcrumb breadcrumb-col-pink ">
            <li><a href="{{ route('admin.add1') }}"> <i class="material-icons">apps</i>Add</a></li>
        </ol>
        <!-- Exportable Table -->
                ...
                <div class="card">
                    ...
                    <div class="body">
                        <div class="table  table-responsive text-center">
                            <table
                                class="table-bordered table-striped table-hover @if(count($add1)>0) dataTable js-exportable @endif">
                                ...
                                <tbody>
                                    @foreach ($add1 as $item)
                                    <tr>
                                        <td><img src="{{ asset('/').$item->image }}" style="width:100px "
                                                alt="Add">
                                        </td>
                                        <td>{{ $item->link }}</td>
                                        <td>
                                        ...
                                        <td>
                                            ...
                                            @if ($item->status==1)
                                            <a href="{{ route('admin.add1.unpublished',['id'=>$item->id]) }}"
                                            ...
                                            @else
                                            <a href="{{ route('admin.add1.published',['id'=>$item->id]) }}"
                                            ...
                                            <a class="btn bg-success btn-info btn-circle waves-effect waves-circle waves-float"
                                                data-toggle="modal" data-target="#editAdd1"
                                                data-id="{{ $item->id }}" data-link="{{ $item->link }}">
                                                <i class="material-icons">edit</i>
                                            </a>
                                            <a id="delete" href="{{ route('admin.add1.delete',['id'=>$item->id]) }}"
...
@include('backend.admin.add.lenght.create')
@include('backend.admin.add.lenght.edit')
@endsection
@push('js')
...
{{-- Update Service  --}}
<script>
    $('#editAdd1').on('show.bs.modal', function(event) {
